<!DOCTYPE html>
<html lang="vi">

<head>
    <meta charset="UTF-8">
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI"
        crossorigin="anonymous"></script>
    <title>Thanh toán</title>
    <link rel="stylesheet" href="../../css/khachhang.css">


</head>

<body>

    <div class="breadcrumb-box">
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb tour-breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="#" class="breadcrumb-link ">Trang chủ</a>
                    </li>
                    <li class="breadcrumb-item">
                        <a href="#" class="breadcrumb-link">Tên tour</a>
                    </li>
                </ol>
            </nav>
        </div>
    </div>

    <div class="container my-4">

        <div class="row">

            <!-- BOX CHỌN THANH TOÁN -->

            <div class="col-lg-6">

                <div class="box">

                    <h5>Chọn phương thức thanh toán</h5>

                    <form method="post">

                        <div class="row">

                            <div class="col-6">
                                <button class="payment-option" name="payment_method" value="momo">
                                    MoMo
                                </button>
                            </div>

                            <div class="col-6">
                                <button class="payment-option" name="payment_method" value="vnpay">
                                    VNPay
                                </button>
                            </div>

                        </div>

                    </form>


                    <!-- HIỂN THỊ QR -->
                    <!-- 
                    <?php if(isset($_POST['payment_method'])): ?>

                    <div class="qr-box">

                        <p class="mt-3">Quét mã QR để thanh toán</p>

                        <?php if($_POST['payment_method']=="momo"): ?>

                        <img src="images/qr-momo.png">

                        <?php else: ?>

                        <img src="images/qr-vnpay.png">

                        <?php endif; ?>


                        <button class="btn btn-pay text-white mt-3" data-bs-toggle="modal"
                            data-bs-target="#successModal">

                            Thanh toán đã hoàn tất

                        </button>

                    </div>

                    <?php endif; ?> 
-->
                </div>

            </div>


            <!-- BOX THÔNG TIN ĐƠN -->

            <div class="col-lg-6">

                <div class="box">

                    <h5 class="text-danger">
                        Đơn đặt của bạn đã được tạm khóa trong 10 phút
                    </h5>

                    <hr>

                    <p><strong>Tour:</strong> Tour Sapa 3N2Đ</p>

                    <p><strong>Ngày khởi hành:</strong> 20/07/2026</p>

                    <p><strong>Số người:</strong> 2</p>

                    <p><strong>Họ tên:</strong> Nguyễn Văn A</p>

                    <p><strong>SĐT:</strong> 0988888888</p>

                    <hr>

                    <h5>
                        <strong>Tổng tiền:</strong> <span class="price">7.980.000đ</span>
                    </h5>

                </div>

            </div>

        </div>

    </div>

    </div>


    <!-- POPUP CẢM ƠN -->

    <div class="modal fade" id="successModal">

        <div class="modal-dialog modal-dialog-centered">

            <div class="modal-content p-4 text-center">

                <h4 class="mb-3">Thanh toán thành công</h4>

                <p>
                    Cảm ơn bạn đã lựa chọn dịch vụ của chúng tôi.
                    Đơn đặt tour của bạn đã được ghi nhận thành công và đang được xử lý.
                    Chúng tôi sẽ liên hệ với bạn trong thời gian sớm nhất để xác nhận chuyến đi.
                </p>

                <a href="khachHang.html" class="btn btn-success mt-3">
                    Quay trở lại trang chủ
                </a>

            </div>

        </div>

    </div>


    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>

</body>

</html>